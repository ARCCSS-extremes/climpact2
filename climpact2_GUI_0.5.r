...
#		tkconfigure(ok.button,text='   Continue    ',bg='white',font=font_small) } # end of previous
	...
	# Check date order
                if(any(dates[1:length(dates)-1] > dates[2:length(dates)])) {
                        test <- tkmessageBox(message = "Dates are out of order, please check dates in your input file.",icon = "warning", title = "ClimPACT - warning")
                        tkfocus(start1)
                        return() }
...
#		prcp <- prcp[!is.na(prcp)]	# combine these two lines
		...
		if(any(tmin.rle$lengths[tmin.rle$values==0] > running.zero.allowed.in.temperature)) {
			...
			beg.index <- end.index-tmin.rle.na$lengths[tmin.rle.na$values==0 & tmin.rle.na$lengths>running.zero.allowed.in.temperature]+1
				...
				cio@dates[beg.index[1]]," and ",cio@dates[end.index[1]],". Please check these, processing will continue.",sep=""),icon = "warning", title = "ClimPACT - warning")
...
# object 'cio'. Thus, the user can repeat this process multiple times to add multiple new columns of data.
	additional.data <- function() {
                if (is.null(cio)) {
                  tkmessageBox(message = paste("Please load climate data first.",sep = ""))
                  return() }

                if (is.null(add.default)) {
...
#	        nama<-substr(add.file.name,start=1,stop=(nchar(dir.file.name)-4))
	        assign('add.file.name',add.file.name,envir=.GlobalEnv)

	# read in data from file
	        add.data <- read.table(add.file.name,header=F,col.names=c("year","month","day","prcp","tmax","tmin"),colClasses=rep("real",6))
...
# Check that years are in ascending order
	if(!all(data$year == cummax(data$year))) {
                tkmessageBox(message = "Years are not in ascending order, please check your input file.",icon = "warning", title = "ClimPACT - warning")
                tkfocus(start1)
                return()
	}
...
# define some error messages
	...
	tkpack(tkbutton(tt1, text = "  Load and QC the data >> ",command = qcontrol, font = fontHeading2, bg = "white"))
        tkpack(frame.space,side='top')
...
#        tkpack(frame.space,side='top')
#        tkpack(tkbutton(tt1, text = "  Load additional data ",command = additional.data, font = font_small, bg = "white"))
#
#        add.data.label<-tklabel(tt1,text='Enter variable name: ',bg='white',font=font_small)
#        tkpack(add.data.label,side='left')
#        add.default <- tclVar('')
#        add.data.name <- tkentry(tt1, width = 10, textvariable = add.default, bg = "white")
#        tkpack(add.data.name, side = 'left')
...
index.calc1 <- function() {
	if(is.null(cio)) {
		tkmessageBox(message = "You must load some data first.",
...
print(base.year.start);			index.store <- climdex.spi(cio,ref.start=c(as.numeric(base.year.start),1),ref.end=c(as.numeric(base.year.end),12)) ; write.precindex.csv(index.store,index.name=indices[52]) ; plot.precindex(index.store,index.name=indices[52],index.units=units[52],x.label="Years") } }
