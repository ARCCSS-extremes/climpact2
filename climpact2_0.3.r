...
# Loop through index list; get index, read variable, calculate index on grid, write out index on gridded netcdf
	...
	# create quantile arrays dimensions [lon,lat,days,percentiles,base]
		if(write_quantiles == TRUE) { tminqtiles_array = array(NA,c(length(lon),length(lat),365,length(tempqtiles),2)) ; tmaxqtiles_array = array(NA,c(length(lon),length(lat),365,length(tempqtiles),2)) ; 
			tavgqtiles_array = array(NA,c(length(lon),length(lat),365,length(tempqtiles),2)) ; precipqtiles_array = array(NA,c(length(lon),length(lat),365,length(precqtiles))) }
				...
				if (write_quantiles == TRUE) { 
					...
					for (l in 1:length(tempqtiles)) {
						...
						tminqtiles_array[i,j,,l,1] = cio@quantiles$tmin$outbase[[l]]
                                                ...
                                                tminqtiles_array[i,j,,l,2] = cio@quantiles$tmin$outbase[[l]]
                                                tmaxqtiles_array[i,j,,l,2] = cio@quantiles$tmax$outbase[[l]]
                                                tavgqtiles_array[i,j,,l,2] = tavgqtiles$tmax$outbase[[l]]
					}
                                        for (l in 1:length(precqtiles)) {
						precipqtiles_array[i,j,,l] = cio@quantiles$prec[[l]]
					}
				}
			}
		}
...
# NOTE: ncdf4 seems to only support numeric types for dimensions.
	...
	# write out qunatiles if requested
		if(write_quantiles == TRUE) {
			...
			tqnames = as.numeric(substr(names(cio@quantiles$tmin$outbase),2,3))
			pqnames = as.numeric(substr(names(cio@quantiles$prec),2,3))

			# create time, quantile and inbase/outbase dimensions
			timedim <- ncdim_def("time","days",0.5:364.5) ; tqdim <- ncdim_def("tqtile","unitless",tqnames) ; pqdim <- ncdim_def("pqtile","unitless",pqnames) ; basedim <- ncdim_def("base","unitless",1:2)

			# create variable ncdf objects
                        tmincdf = ncvar_def(paste("tmin",sep=""),"C",list(londim,latdim,timedim,tqdim,basedim),-1,prec="float")
                        tmaxcdf = ncvar_def(paste("tmax",sep=""),"C",list(londim,latdim,timedim,tqdim,basedim),-1,prec="float")
                        tavgcdf = ncvar_def(paste("tavg",sep=""),"C",list(londim,latdim,timedim,tqdim,basedim),-1,prec="float")
