...
load.data.qc <- function() {
	...
	# If quantiles have not been read in, then calculate them if necessary and write out to file
		...
		if(!is.null(temp.quantiles)) {
			...
			nam2 <- paste(outthresdir, paste(ofilename, "_thres_spei.csv", sep = ""),sep="/")
	                write.table(as.data.frame(thres2), file = nam2, append = FALSE, quote = FALSE, sep = ", ", na = "-99.9", col.names = c("Base period dates","Base period tmin","Base period tmax","Base period prec"),row.names=FALSE)
		} else { 
                        tn90p <- hwlist[[2]]
                        tx90p <- hwlist[[1]]
                        tavg90p <- hwlist[[3]]
	...
	qcontrol <- function() {
                ...
                yyymmdd <- paste(data[,1],data[,2],data[,3],sep="-")
                dates <- as.Date(yyymmdd,format="%Y-%m-%d")
                base.year.start<<-as.numeric(tclvalue(base.year.start.tcl))
                base.year.end<<-as.numeric(tclvalue(base.year.end.tcl))

		# arbitrarily change base period to be outside of the date range of the loaded data, if quantiles are loaded. This is a kludge to prevent the code getting confused later on and does not affect the output.
		...
		if(is.null(quantiles)) {
				...
				tkmessageBox(message = paste("Base period must be between ", format(dates[1],format="%Y")," and ",format(dates[length(dates)],format="%Y"),". Please correct.",sep="")) ; close(process.pb);return() }
		}
...
index.calc2<-function(){
	...
	index.calc3 <- function(){
		...
		if (cbv[31]==1) { print(paste("calculating",indices[31])) ; index.store <- climdex.cddcold(cio,Tb=Tb_CDD) ; write.index.csv(index.store,index.name=indices[31]) ; plot.call(index.store,index.name=indices[31],index.units=units[31],x.label="Years")
                ...
                cat(file=trend_file,paste(latitude,longitude,indices[49],years,yeare,round(as.numeric(out$coef.table[[1]][2, 1]), 3),round(as.numeric(out$coef.table[[1]][2, 2]), 3),round(as.numeric(out$summary[1, 6]),3),sep=","),fill=180,append=T) }
                if (cbv[50]==1) { print(paste("calculating",indices[50])) ; if(all(is.na(cio@data$tmin)) | all(is.na(cio@data$tmax))) { warning("NOT PLOTTING HEATWAVE INDICES: Requires tmin and tmax.") } else {
			...
			# If heatwave previous percentiles have been read in by user then use these in heatwave calculations, otherwise let climdex.hw calculate percentiles using currently loaded data.
                        if(!is.null(cio@quantiles$hwtmax) && !is.null(cio@quantiles$hwtmin)) { tx90p <- cio@quantiles$hwtmax$q90 ; tn90p <- cio@quantiles$hwtmin$q90 ; tavg90p <- cio@quantiles$hwtavg$q90 } else {
				...
				write.hw.csv(index.store,index.name=indices[50])
				plot.hw(index.store,index.name=indices[50],index.units=units[50],x.label="Years") 
			} }
                if (cbv[51]==1) { print(paste("calculating",indices[51])) ; if(all(is.na(cio@data$tmin)) | all(is.na(cio@data$tmax)) | all(is.na(cio@data$prec))) { warning("NOT PLOTTING SPEI: climdex.spei REQUIRES TMIN, TMAX AND PRECIP DATA.") } else {
			setTkProgressBar(pb,95,title="95%")
			# If SPEI/SPI thresholds have been read in by user then use these in SPEI/SPI calculations.
			if(exists("speidata")) {# && !is.null(speidata$speitmax) && !is.null(speidata$speitmin) && !is.null(speidata$speiprec) && !is.null(speidata$speidates)) {
				tnraw <- speidata$speitmin ; txraw <- speidata$speitmax ; praw <- speidata$speiprec ; btime <- speidata$speidates } else {
				...
				dat = ts(prec_sum-pet,freq=12,start=c(as.numeric(date.years[1]),1),end=c(as.numeric(date.years[length(date.years)]),12))
				...
				write.precindex.csv(index.store,index.name=indices[51])
				plot.precindex(index.store,index.name=indices[51],index.units=units[51],x.label="Years") } }
			...
			setTkProgressBar(pb,99,title="99%")
				...
				dat <- ts(prec_sum,freq=12,start=c(as.numeric(date.years[1]),1),end=c(as.numeric(date.years[length(date.years)]),12))
					...
					cspi(dat,na.rm=T,scale=12,ref.start=c(base.year.start,1),ref.end=c(base.year.end,1))$fitted),c(length((cspi(prec_sum,na.rm=T,scale=c(3))$fitted)),3))
	                        index.store <- aperm(index.store,c(2,1))
	                        index.store <- ifelse(index.store=="Inf" | index.store=="-Inf" | index.store=="NaN",NA,index.store)
				write.precindex.csv(index.store,index.name=indices[52])
				plot.precindex(index.store,index.name=indices[52],index.units=units[52],x.label="Years") } }
...
write.precindex.csv <- function(index=NULL,index.name=NULL) {
	...
	colnames <- list("time",index.name)
        ...
        write.table(cbind(unique(as.character(cio@date.factors$monthly)),index[1,]), file = nam1, append = TRUE, quote = FALSE, sep = ", ", na = "-99.9", row.names=FALSE,col.names = FALSE)

        # write 6 month data
        nam1 <- paste(outinddir, paste(ofilename, "_6month_",index.name,".csv", sep = ""), sep = "/")
        write.table(colnames, file = nam1, append = FALSE, quote = FALSE, sep = ", ", na = "-99.9", row.names=FALSE,col.names = FALSE)
        write.table(cbind(unique(as.character(cio@date.factors$monthly)),index[2,]), file = nam1, append = TRUE, quote = FALSE, sep = ", ", na = "-99.9", row.names=FALSE,col.names = FALSE)

        # write 12 month data
        nam1 <- paste(outinddir, paste(ofilename, "_12month_",index.name,".csv", sep = ""), sep = "/")
        write.table(colnames, file = nam1, append = FALSE, quote = FALSE, sep = ", ", na = "-99.9", row.names=FALSE,col.names = FALSE)
        write.table(cbind(unique(as.character(cio@date.factors$monthly)),index[3,]), file = nam1, append = TRUE, quote = FALSE, sep = ", ", na = "-99.9", row.names=FALSE,col.names = FALSE)
}
...
plot.precindex <- function(index=NULL,index.name=NULL,index.units=NULL,x.label=NULL) {
	...
	scales = c(3,6,12)
        ...
        for (time in 1:3) {
	        ...
	        jpeg(file = namp, width = 1024, height = 768)
	        dev0 = dev.cur()
	        plotx(unique(as.character(cio@date.factors$monthly)), index[time,], main = gsub('\\*', index.units, plot.title),ylab = index.units,xlab = x.label)
                ...
                fit1<-suppressWarnings(lsfit(1:length(date.months),index[time,]))
                out1<<-ls.print(fit1,print.it=F)
