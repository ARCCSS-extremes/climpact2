...
latstr <<- latdim <<- londim <<- j <<- tsmin <<- tsmax <<- prec <<- tsmintime <<- tsmaxtime <<- prectime <<- missingval <<- cicompile <<- tminqtiles_array <<- tmaxqtiles_array <<-  
...
ntxbntnb_n=5,gslmode="GSL",rx5day_centermean=FALSE,hddheat_n=18,cddcold_n=18,gddgrow_n=10,time_format=NULL,rxnday_n=5,rxnday_center.mean.on.last.day=FALSE,rnnm_threshold=1,
...
# Read in climate index data. Do indices check.
	indexfile = "/home/z3506872/hdrive/scripts/climpact2/source/index.master.list"
...
# Loop through index list; get index, read variable, calculate index on grid, write out index on gridded netcdf
	...
	# Create index call string (better way to do this? Somehow to use |...). For performance reasons we have the following switch command to, among other things, reduce the number of percentiles to calculate for each index.
		...
		switch(indices[a],
			...
			gsl={# lat.tmp = eval(parse(text=latstr));print(lat.tmp) ;print(typeof(lat.tmp)); if(lat.tmp<= 0){print("SH"); northern.hemisphere = FALSE }else northern.hemisphere = TRUE
				indexparam = paste("array(indexcompile(",indexparam,",gsl.mode=",dQuote(gslmode),"))",sep="") ; if(!write_quantiles) {tempqtiles_tmp = c(0.1) ; precqtiles_tmp = c(0.1) } },
			...
			spei={indexparam = paste("array(indexcompile(",indexparam,",scale=",spei_scale,",lat=",latstr,",ref.start=",paste("c(",baserange[1],",1)",sep=""),
				",ref.end=",paste("c(",baserange[2],",1)","))",sep="")) ; if(!write_quantiles) {tempqtiles_tmp = c(0.1) ; precqtiles_tmp = c(0.1) } },
                        spi={indexparam = paste("array(indexcompile(",indexparam,",scale=",spi_scale,",ref.start=",paste("c(",baserange[1],",1)",sep=""),",ref.end=",paste("c(",
				baserange[2],",1)","))",sep="")) ; if(!write_quantiles) {tempqtiles_tmp = c(0.1) ; precqtiles_tmp = c(0.1) } },
			hw={	indexparam = paste("array(indexcompile(",indexparam,",base.range=c(",baserange[1],",",baserange[2],"),pwindow=",hwn_n,",min.base.data.fraction.present=",
                                min.base.data.fraction.present,",lat=",latstr,",tavg90p=tavg90p",",tn90p=tn90p",",tx90p=tx90p","))",sep="") ; if(!write_quantiles) {tempqtiles_tmp = c(0.1,0.9) ; precqtiles_tmp = c(0.1,0.9) } },
	...
	# Calculate the index. Parallelise outer loop.
		...
		index <- foreach(j=1:length(lat),.combine='acomb',.export=c(ls(envir=globalenv()),objects(),"get.na.mask","tapply.fast"),.packages=c('climdex.pcic','SPEI')) %dopar% {		# ls(envir=globalenv()),objects()
				...
				if(indices[a] == "hw") { if(!is.null(quantile_file)) { tavg90p = tavgqtileshw[i,j,] ; tn90p = tminqtileshw[i,j,] ; tx90p = tmaxqtileshw[i,j,] } else { tavg90p = NULL ; tx90p = NULL ; tn90p = NULL}
					test[,,i,] = eval(parse(text=indexparam)) }
				else if(indices[a] == "spei" | indices[a] == "spi") { test[,i,] = eval(parse(text=indexparam)) }
				else if(indices[a] == "gsl") { if(eval(parse(text=latstr)) <= 0) {print("SH") ; print(eval(parse(text=latstr))); cio@northern.hemisphere<<-FALSE} else cio@northern.hemisphere<<-TRUE ;test[i,] = eval(parse(text=indexparam))}
...
# NOTE: ncdf4 seems to only support numeric types for dimensions.
	...
	# write out global attributes from input file. Assumes all input files have the same global attributes.
		...
		for(i in 1:length(globatt)) { if(names(globatt)[i] == "date" || names(globatt)[i] == "author" && names(globatt)[i] == "contact" || names(globatt)[i] == "creation_date") {} else ncatt_put(tmpout,0,names(globatt)[i],globatt[[i]]) }
        ...
        if(!is.null(tsmin) && !is.null(tsmax)) {
                qtilefetch<-foreach(j=1:length(lat),.combine='acomb_qtile',.export=c(ls(envir=globalenv()),objects(),"get.outofbase.quantiles","cicompile"),.packages='climdex.pcic') %dopar% {
                        for(i in 1:length(lon)){
                                cio = cicompile(tmin=tsmin[i,j,],tmax=tsmax[i,j,],prec=NULL,tmin.dates=tsmintime,tmax.dates=tsmaxtime,prec.dates=NULL,prec.qtiles=NULL,
                                temp.qtiles=tempqtiles_tmp,quantiles=NULL,base.range=baserange)
				tx90p = get.outofbase.quantiles(cio@data$tmax,tmin=NULL,prec=NULL,tmax.dates=cio@dates,tmin.dates=NULL,prec.dates=NULL,base.range=baserange,temp.qtiles=c(0.9),prec.qtiles=NULL,n=15)
                                tavg90p_tn90p = get.outofbase.quantiles(cio@data$tavg,cio@data$tmin,tmax.dates=cio@dates,tmin.dates=cio@dates,base.range=baserange,temp.qtiles=c(0.9),prec.qtiles=NULL,n=15)
...
# Write quantiles to file
        ...
        qout = nc_create(qfile,list(tmincdf,tmaxcdf,tavgcdf,preccdf,tmincdfhw,tmaxcdfhw,tavgcdfhw),force_v4=TRUE)
	...
	nc_close(qout)
        rm(timedim,tqdim,pqdim,tmincdf,tmaxcdf,preccdf,tavgcdfhw,tmaxcdfhw,tmincdfhw,qout)
}
