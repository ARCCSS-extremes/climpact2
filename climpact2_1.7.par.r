...
yeardate <<- origin <<- latstr <<- latdim <<- londim <<- j <<- tsmin <<- tsmax <<- prec <<- tsmintime <<- tsmaxtime <<- prectime <<- missingval <<- cicompile <<- tminqtiles_array <<- tmaxqtiles_array <<-  
northern.hemisphere <<- tavgqtiles_array <<- precipqtiles_array <<- NULL
...
# INSERT basic temp and prec checks (in a function)
...
print(length(monthdate))
print(length(yeardate))
...
# Loop through index list; get index, read variable, calculate index on grid, write out index on gridded netcdf
	...
	# Create index call string (better way to do this? Somehow to use |...). For performance reasons we have the following switch command to, among other things, reduce the number of percentiles to calculate for each index.
		...
		switch(indices[a],
			...
			gsl={# lat.tmp = eval(parse(text=latstr));print(lat.tmp) ;print(typeof(lat.tmp)); if(lat.tmp<= 0){print("SH"); northern.hemisphere = FALSE }else northern.hemisphere = TRUE
				indexparam = paste("array(indexcompile(",indexparam,",gsl.mode=",dQuote(gslmode),"))",sep="") ; if(!write_quantiles) {tempqtiles_tmp = NULL ; precqtiles_tmp = NULL } },
...
print(str(index))
print(str(cio))
print(cio@dates[length(cio@dates)])
print("TMP")
tmp = eval(parse(text=indexparam))
print(str(tmp))
				...
				if(indices[a] == "hw") { if(exists("tavgqtileshw")) { tavg90p = tavgqtileshw[i,j,] ; tn90p = tminqtileshw[i,j,] ; tx90p = tmaxqtileshw[i,j,] } else { tavg90p = NULL ; tx90p = NULL ; tn90p = NULL}
					...
					print(str(test)) ; test[,i,] = eval(parse(text=indexparam)) }
...
# If temp variables exist, prepare arrays to store heat wave quantiles
	...
	innerthw = array(NA,c(3,1,length(lon),365))	# three variables (tx,tn,tavg), and only 90th percentile for each at each lat/lon
        innertminmaxprec = array(NA,c(3,1,length(lon),length(tsmax[1,1,which(yeardate2 >= baserange[1] & yeardate2 <= baserange[2])])))		#which(time>=as.Date(paste(baserange[1],"01","01",sep="-")])))
...
# Recrod tmin, tmax and precip for future spei/spi calcs. Only do if precip exists since spei/spi is not possible without it.
        if(!is.null(prec)) {
                qtilefetch<-foreach(j=1:length(lat),.combine='acomb_qtile',.export=c(ls(envir=globalenv()),objects(),"cicompile"),.packages='climdex.pcic') %dopar% {
                        for(i in 1:length(lon)){
                                cio = cicompile(tmin=tsmin[i,j,],tmax=tsmax[i,j,],prec=NULL,tmin.dates=tsmintime,tmax.dates=tsmaxtime,prec.dates=NULL,prec.qtiles=NULL,
                                temp.qtiles=tempqtiles_tmp,quantiles=NULL,base.range=baserange)
                                innertminmaxprec[1,1,i,] = tsmin[i,j,which(yeardate2 >= baserange[1] & yeardate2 <= baserange[2])]		#tavg90p_tn90p$tmin$outbase$q90 #  cio@quantiles$tmin$outbase[[l]]
                                innertminmaxprec[2,1,i,] = tsmax[i,j,which(yeardate2 >= baserange[1] & yeardate2 <= baserange[2])]		#tx90p$tmax$outbase$q90  #cio@quantiles$tmax$outbase[[l]]
                                innertminmaxprec[3,1,i,] = prec[i,j,which(yeardate2 >= baserange[1] & yeardate2 <= baserange[2])]
                        }
                        innertminmaxprec	 # return the array
                }
                base.tmin.tmax.prec = aperm(qtilefetch,c(3,5,4,2,1))
                rm(qtilefetch)
        }
...
# Write quantiles to file
	...
	tminrawcdf = ncvar_def(paste("tminraw",sep=""),"degC",list(londim,latdim,basetimedim),missingval,prec="double")
        ...
        precrawcdf = ncvar_def(paste("precraw",sep=""),"degC",list(londim,latdim,basetimedim),missingval,prec="double")
...
#  Concatenate base period tmin, tmax and prec that must be passed by user. Beginning of the time series will have base period data, end of the time series will have data from ci, any data in the middle will remain NA.
#  This is done because SPEI needs the base period data to use as a reference and does not have an option to read this in separately. Thus, we construct this synthetic time series that dates from the base period to the end of the 
...
# If the data in 'ci' includes the base period (computefuture is false) then skip this step.
        if(computefuture){
		pcicbasetime = as.PCICt(basetime*3600,cal="gregorian",origin)		# put base period dates in PCICt format
...
climdex.spi <- function(ci,scale=c(3,6,12),kernal=list(type='rectangular',shift=0),distribution='Gamma',fit='ub-pwm',ref.start=NULL,ref.end=NULL,lat=NULL) {
        scale <- c(3,6,12)      # hard-coded for Climpact2 definition.
        stopifnot(is.numeric(scale),all(scale>0),!is.null(ci@data$prec))
...
# step 1. Get data needed for the three definitions of a heat wave. 
	...
	if(any(is.null(tavg90p),is.null(tx90p),is.null(tn90p))) {print("NULLINHW");
