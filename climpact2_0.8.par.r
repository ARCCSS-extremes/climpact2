...
# Loop through index list; get index, read variable, calculate index on grid, write out index on gridded netcdf
	...
	# create quantile arrays if quantile file is requested. [lon,lat,days,percentiles,base]
        ...
        # If quantiles are requested, record quantiles. This will only happen once.
                if (write_quantiles == TRUE) { record.quantiles(tempqtiles,precqtiles,lat,lon,baserange) }
...
#print(objects())
		...
		index <- foreach(j=1:length(lat),.combine='acomb',.export=c(ls(envir=globalenv()),objects()),.packages=c('climdex.pcic','SPEI')) %dopar% {		# ls(envir=globalenv()),objects()
...
record.quantiles <- function(tempqtiles,precqtiles,lat,lon,baserange) {
...
climdex.tm5a <- function(ci) { stopifnot(!is.null(ci@data$tmin)); return(number.days.op.threshold(ci@data$tavg, ci@date.factors$annual, 5, ">=") * ci@namasks$annual$tmin) }

# tm5b
# Annual count when TM < 5ºC
# same as climdex.tr except < 5C
climdex.tm5b <- function(ci) { stopifnot(!is.null(ci@data$tmin)); return(number.days.op.threshold(ci@data$tavg, ci@date.factors$annual, 5, "<") * ci@namasks$annual$tmin) }

# tm10a
# Annual count when TM >= 10ºC
# same as climdex.tr except >=10C
climdex.tm10a <- function(ci) { stopifnot(!is.null(ci@data$tmin)); return(number.days.op.threshold(ci@data$tavg, ci@date.factors$annual, 10, ">=") * ci@namasks$annual$tmin) }

# tm10b
# Annual count when TM < 10ºC
# same as climdex.tr except <10C
climdex.tm10b <- function(ci) { stopifnot(!is.null(ci@data$tmin)); return(number.days.op.threshold(ci@data$tavg, ci@date.factors$annual, 10, "<") * ci@namasks$annual$tmin) }
...
climdex.su35 <- function(ci) { stopifnot(!is.null(ci@data$tmin)); return(number.days.op.threshold(ci@data$tmax, ci@date.factors$annual, 35, ">=") * ci@namasks$annual$tmin) }
...
climdex.hddheat <- function(ci,Tb=18) { 
        stopifnot(is.numeric(Tb)) ; 
	Tbarr = array(Tb,length(ci@data$tavg)); 
	ci@data$tavg = ifelse(ci@data$tavg >= Tbarr,NA,ci@data$tavg)
	...
	return(tapply.fast(Tbarr - ci.tmp@data$tavg,ci.tmp@date.factors$annual,sum))*ci.tmp@namasks$annual }
...
climdex.cddcold <- function(ci,Tb=18) { 
	...
	Tbarr = array(Tb,length(ci@data$tavg)) ; 
        ci@data$tavg = ifelse(ci@data$tavg <= Tbarr,NA,ci@data$tavg)
        ci.tmp = cicompile(tavg=ci@data$tavg,tavg.dates=ci@dates,base.range=c(as.numeric(format(as.Date(as.character(ci@base.range[1])),format="%Y")),as.numeric(format(as.Date(as.character(ci@base.range[2])),format="%Y"))))
	return(tapply.fast(ci.tmp@data$tavg - Tbarr,ci.tmp@date.factors$annual,sum))*ci.tmp@namasks$annual }
...
climdex.ntxbntnb <- function(ci, n=5) {
        stopifnot(!is.null(ci@data$tmax) && !is.null(ci@quantiles$tmax) || (!is.null(ci@data$tmin) && !is.null(ci@quantiles$tmin)))	# TODO: This could be changed to a stopifnot(any( statement... do it
...
#	ind <- runlength$lengths <= k & runlength$values==FALSE	# get indices where non-events <= k days long
#	runlength[ind] = TRUE					# make these non-events into events, so that the true events prior to and after this non-event are counted as one.
